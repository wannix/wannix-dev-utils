...
export const FLAGS: RegexFlag[] = [
    ...
    { key: 'i', label: 'Case Insensitive', description: 'Case-insensitive matching' },
...
export const PRESETS: RegexPreset[] = [
    ...
    { label: 'Phone (NZ)', pattern: '(?:\\+64|0)(?:2[0-9]|3|4|6|7|9)\\d{6,8}', flags: 'g' },
    { label: 'Date (YYYY-MM-DD)', pattern: '\\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\\d|3[01])', flags: 'g' },
    { label: 'IPv4', pattern: '(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\.(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\.(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\.(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)', flags: 'g' },
    { label: 'Hex Color', pattern: '#(?:[0-9a-fA-F]{3}){1,2}', flags: 'gi' },
]

export function findMatches(pattern: string, flags: string, testString: string): { matches: RegexMatch[]; error?: string } {
