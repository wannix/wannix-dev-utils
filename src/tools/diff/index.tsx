...
import { RotateCcw, Split, FileDiff, Type, AlignLeft, Braces } from 'lucide-react'
        ...
        >
            <div className="flex flex-col h-[calc(95vh-14rem)] min-h-[600px] gap-6">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-muted/20 p-2 rounded-lg border border-border/40">
                    <Tabs value={mode} onValueChange={(v) => setMode(v as DiffMode)} className="w-full sm:w-auto">
                        <TabsList className="bg-background border border-border/50">
                            <TabsTrigger value="chars" className="gap-2"><Type className="h-4 w-4" /> Chars</TabsTrigger>
                            <TabsTrigger value="words" className="gap-2"><AlignLeft className="h-4 w-4" /> Words</TabsTrigger>
                            <TabsTrigger value="lines" className="gap-2"><Split className="h-4 w-4" /> Lines</TabsTrigger>
                            <TabsTrigger value="json" className="gap-2"><Braces className="h-4 w-4" /> JSON</TabsTrigger>
                        </TabsList>
                    </Tabs>

                    <Button variant="ghost" size="sm" onClick={handleClear} className="text-muted-foreground hover:text-foreground gap-2">
                    ...
                    <div className="flex flex-col gap-4 min-h-0 h-full">
                        <Card className="flex flex-col flex-1 min-h-0 border-border/50 bg-card shadow-sm overflow-hidden">
                            <CardHeader className="py-2 px-3 border-b border-border/40 bg-muted/20 shrink-0">
                                <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Original Text</CardTitle>
                            </CardHeader>
                        ...
                        <Card className="flex flex-col flex-1 min-h-0 border-border/50 bg-card shadow-sm overflow-hidden">
                            <CardHeader className="py-2 px-3 border-b border-border/40 bg-muted/20 shrink-0">
                                <CardTitle className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Modified Text</CardTitle>
                            </CardHeader>
                            <CardContent className="p-0 flex-1">
                                <Textarea
                                    className="h-full w-full font-mono text-sm resize-none border-0 focus-visible:ring-0 rounded-none bg-transparent p-4 leading-relaxed"
                            ...
                            {diffResult.length > 0 ? (
                                <div className="p-4 font-mono text-sm whitespace-pre-wrap break-all leading-relaxed">
                                    {diffResult.map((part, index) => {
                                        const color = part.added ? 'bg-green-500/20 text-green-700 dark:text-green-300 border-green-500/30' :
                                            part.removed ? 'bg-red-500/20 text-red-700 dark:text-red-300 border-red-500/30' :
                                        ...
                                        const style = part.added || part.removed ? { borderRadius: '2px', padding: '0 2px' } : {};
