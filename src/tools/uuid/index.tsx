...
export default function UuidTool() {
  ...
  const generate = (saveToHistory = true) => {
    const newUuids = generateIds(idType, quantity, {
      hyphens,
      uppercase,
      braces,
    });
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    {idType === "uuid-v4" &&
                      "Universally Unique Identifier, 128-bit random."}
                    {idType === "ulid" &&
                      "Universally Unique Lexicographically Sortable Identifier."}
                    {idType === "ksuid" &&
                      "K-Sortable Unique IDentifier, base62 encoded."}
                  </p>
                </div>
              ...
              <div className="h-full overflow-y-auto p-2">
                <div className="space-y-1">
                  {uuidList.map((uuid, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-3 p-2 rounded hover:bg-muted/40 group font-mono text-sm border border-transparent hover:border-border/30"
                    >
                      <span className="text-muted-foreground w-8 text-right text-xs select-none">
                        #{idx + 1}
                      </span>
