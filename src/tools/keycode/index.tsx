          ...
          {eventType ? (
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                Keyboard works anywhere • Mouse/touch events in this zone
              </div>
            </div>
          )}
        </div>
                ...
                {history.length > 0 ? (
                  <ul className="divide-y divide-border/30">
                    {history.map((h, i) => (
                      <li
                        key={`${h.timestamp}-${i}`}
                        className={`flex items-center justify-between py-2 px-4 ${i === 0 ? "bg-primary/5" : ""}`}
                      >
                        <div className="flex items-center gap-3">
                          <span
                            className={`text-[10px] font-semibold uppercase tracking-wider ${TYPE_COLORS[h.type]}`}
                          >
                            {TYPE_LABELS[h.type].slice(0, 3)}
                          </span>
                          <span className="font-mono text-sm font-medium w-20 truncate">
                            {h.label}
                          </span>
                        </div>
