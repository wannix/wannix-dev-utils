...
const DataDisplay = ({
  ...
  return (
    <Card
      className={`border-border/50 bg-card/50 shadow-sm overflow-hidden flex flex-col ${className}`}
    >
      <CardHeader
        className={`px-4 py-2 border-b border-border/40 ${colorClass} bg-opacity-10 shrink-0`}
      >
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-xs font-semibold tracking-wide uppercase text-foreground/90">
              {title}
            </CardTitle>
          ...
          <div className="flex items-center gap-4">
            <Tabs
              value={view}
              onValueChange={(v) => setView(v as "json" | "table")}
              className="h-7"
            >
              <TabsList className="h-7 bg-background/50 border border-border/20">
                <TabsTrigger
                  value="json"
                  className="h-5 text-[10px] px-2 data-[state=active]:bg-muted"
                >
                  JSON
                </TabsTrigger>
                <TabsTrigger
              ...
              <TableBody>
                {Object.entries(data).map(([key, value]) => {
                  ...
                  if (isTimestamp) {
                    ...
                    if (key === "alg" || key === "typ") {
                      displayValue = (
                        <span className="text-red-500/90">{String(value)}</span>
                      );
                    } else if (["iss", "sub", "aud", "jti"].includes(key)) {
                      displayValue = (
                        <span className="text-purple-500/90">
                          {String(value)}
                        </span>
                      );
                    }
                  ...
                  return (
                    ...
                    >
                      <TableCell className="font-mono text-xs font-medium text-muted-foreground py-2">
                        {key}
                      </TableCell>
                      ...
                      <TableCell className="py-2">
                        <Button
                          size="icon"
                          variant="ghost"
                          className="h-4 w-4 opacity-0 group-hover:opacity-50 hover:!opacity-100"
                          title="Description"
                        >
        ...
        <div className="flex flex-col gap-0 lg:w-[38%] h-full bg-card border border-border/40 rounded-lg overflow-hidden flex-shrink-0">
          ...
          <div className="flex-1 relative p-4 flex flex-col gap-4 overflow-y-auto">
            ...
            {decoded?.error && token && (
              <Alert
                variant="destructive"
                className="mt-2 border-destructive/50 bg-destructive/10"
              >
